...
import {genChartByAiUsingPostAsync} from "@/services/DETS/chartController";
...
const AddChartAsync: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      const res = await genChartByAiUsingPostAsync(params, {}, values.file.file.originFileObj);
      // 正常情况下，如果没有返回值就分析失败，有，就分析成功
      if (!res?.data) {
        message.error('分析失败');
      } else {
        message.success('分析成功,请稍后在我的图表页面查看结果');
        form.resetFields();
      }
      // 异常情况下，提示分析失败+具体失败原因
    } catch (e: any) {
